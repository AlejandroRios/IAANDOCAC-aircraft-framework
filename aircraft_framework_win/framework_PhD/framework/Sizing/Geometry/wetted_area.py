...
def wetted_area(vehicle):
    ...
    engine_thrust, _ = turbofan(
        0, 0, 1, engine)  # force [N], fuel flow [kg/hr]
    ...
    fuselage['tail_length'] = tailcone_sizing(
        aircraft['passenger_capacity'], engine['position'], fuselage_diameter, fuselage_diameter)
    ...
    # --> Fuselagem dianteira
    fuselage_wetted_area_forward = wetted_area_forward_fuselage(
        fuselage)
    ...
    wing_sweep_trailing_edge = 1/deg_to_rad*(np.arctan(np.tan(deg_to_rad*wing['sweep_c_4'])-3/wing_trap_aspect_ratio*(
    ...
    wing['kink_chord'] = (wing['span']/2*np.tan(deg_to_rad*wing['sweep_leading_edge'])+wing['tip_chord'])-(wing_kink_chord_yposition*np.tan(deg_to_rad *
                                                                                                                                            wing['sweep_leading_edge'])+(wing['span']/2-wing_kink_chord_yposition)*np.tan(deg_to_rad*wing_sweep_trailing_edge))  # [m] corda da quebra
    wing_trap_root_chord = (wing['span']/2*np.tan(deg_to_rad*wing['sweep_leading_edge'])+wing['tip_chord'])-(wing_root_chord_yposition*np.tan(deg_to_rad *
                                                                                                                                              wing['sweep_leading_edge'])+(wing['span']/2-wing_root_chord_yposition)*np.tan(deg_to_rad*wing_sweep_trailing_edge))  # [m] corda da raiz fus trap
    ...
    wing_aileron_chord = (wing['span']/2*np.tan(deg_to_rad*wing['sweep_leading_edge'])+wing['tip_chord'])-((0.75*wing['span']/2)*np.tan(deg_to_rad *
                                                                                                                                        wing['sweep_leading_edge'])+(wing['span']/2-(0.75*wing['span']/2))*np.tan(deg_to_rad*wing_sweep_trailing_edge))  # corda no aileron
    wing_aileron_surface = (wing['root_chord']+wing['kink_chord'])*(wing_kink_chord_yposition-wing_root_chord_yposition)+(wing['kink_chord'] +
                                                                                                                          wing_aileron_chord)*((0.75*wing['span']/2)-wing_kink_chord_yposition)  # area exposta com flap
    ...
    vertical_tail['sweep_leading_edge'] = 1/deg_to_rad*(np.arctan(np.tan(deg_to_rad*vertical_tail['sweep_leading_c_4'])+1/vertical_tail['aspect_ratio']
                                                                  * (1-vertical_tail['taper_ratio'])/(1+vertical_tail['taper_ratio'])))  # [�] enflechamento bordo de ataque
    vertical_tail_sweep_c_2 = 1/deg_to_rad*(np.arctan(np.tan(deg_to_rad*vertical_tail['sweep_leading_c_4'])-1 /
                                                      vertical_tail['aspect_ratio']*(1-vertical_tail['taper_ratio'])/(1+vertical_tail['taper_ratio'])))  # [�] enflechamento C/2
    vertical_tail_sweep_trailing_edge = 1/deg_to_rad*(np.arctan(np.tan(deg_to_rad*vertical_tail['sweep_leading_c_4'])-3/vertical_tail['aspect_ratio']
    ...
    if engine['position'] == 1:
        ...
        wing_engine_external_position_chord = (wing['span']/2*np.tan(deg_to_rad*wing['sweep_leading_edge'])+wing['tip_chord'])-(wing_engine_external_yposition
                                                                                                                                * np.tan(deg_to_rad*wing['sweep_leading_edge'])+(wing['span']/2-wing_engine_external_yposition)*np.tan(deg_to_rad*wing_sweep_trailing_edge))
