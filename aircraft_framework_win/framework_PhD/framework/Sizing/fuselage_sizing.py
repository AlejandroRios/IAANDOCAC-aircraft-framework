...
def fuselage_cross_section(container_type, aisles_number, seats_number, cabin_height,
    ...
    w4 = number_of_seats_abreast*seat_cushion_width_YC + aisles_number * \
        aisle_width + (number_of_seats_abreast -
                       aisles_number + 1)*armrest_width
    ...
    while iterations > 0:
        ...
        while k <= points_number:
            ...
            a0 = np.sqrt(((w0/2)**2 + h0**2)/(height_to_width_ratio)**2)
            a1 = np.sqrt(((w1/2)**2 + h1**2)/(height_to_width_ratio)**2)
            a2 = np.sqrt(((w2/2)**2 + h2**2)/(height_to_width_ratio)**2)
            a3 = np.sqrt(((w3+0.04)/2)**2 +
                         ((h3**2)/(height_to_width_ratio**2)))
            a4 = np.sqrt(((w4+0.04)/2)**2 +
                         ((h4**2)/(height_to_width_ratio**2)))
            a5 = np.sqrt(((w5/2)**2 + h5**2)/(height_to_width_ratio)**2)
            a6 = np.sqrt(((w6/2)**2 + h6**2)/(height_to_width_ratio)**2)
            a7 = np.sqrt(((w7/2)**2 + h7**2)/(height_to_width_ratio)**2)
            a8 = np.sqrt(((w8/2)**2 + h8**2)/(height_to_width_ratio)**2)
            a9 = np.sqrt(((w9/2)**2 + h9**2)/(height_to_width_ratio)**2)

            # Get the maximum value of these widths, so each point is inside
            # the fuselage
            array_widths = [a0, a1, a2, a3, a4, a5, a6, a7, a8, a9]
            maximum_width = max(array_widths)

            # If the current width is one of the 2 smallest, then it has to be stored
        ...
        fuselage_equivalent_diameter = 2 * \
            minimum_width*np.sqrt(height_to_width_ratio)
    ...
    axis_y = axis_x*height_to_width_ratio

    axis_x_exterior = axis_x + 2*fuselage_thickness
    axis_y_exterior = axis_y + 2*fuselage_thickness

    fuselage_width = axis_x_exterior
    fuselage_height = axis_y_exterior

    return fuselage_width, fuselage_height, fuselage_Dz_floor, minimum_width
# =============================================================================
# MAIN
# =============================================================================
