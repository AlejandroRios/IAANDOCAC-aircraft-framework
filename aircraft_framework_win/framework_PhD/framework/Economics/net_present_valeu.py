...
def total_non_recurrig_year_cost():
    ...
    total_hours =  manegment_and_administration_hours() + engineering_hours() + test_and_validation() + support_hours()
    ...
    infrastructure_cost_vector = [i * infrastructure_cost for i in infrastructure_distribution]
    ...
    for i in range(len(general_cost_vector)):
        total_non_recurring_cost_aux = manufacturer*(man_power_cost_vector[i] + infrastructure_cost_vector[i] + general_cost_vector[i])
...
def material():
    ...
    materials_cost = (materials['landing_gear'] * complexity_factors['landing_gear'] * MTOW +
                materials['flight_controls'] * complexity_factors['flight_controls'] * wing_surface_ft  +
                materials['engines'] * complexity_factors['engines'] * engines_number * thrust_factor +
                materials['naceles'] * complexity_factors['nacelles'] * engine_diameter_in +
                materials['interior'] * complexity_factors['interior'] * pax_number +
                materials['electrical_system'] * complexity_factors['electrical'] * KVA +
                materials['avionics'] * complexity_factors['avionics'] +
                materials['structures'] * complexity_factors['structures'] * MTOW + 
...
def man_power():
    ...
    LCPH = 25
    baseline = [0,0,0,0,36714,26452,23363,21451,20218,19416,18899,18581,18389,18281,18173]

    recurring_man_power_cost = []
    for i in range(p):
        recurring_man_power_cost_aux = baseline[i] * delivers[i] * MTOW_factor * LCPH
...
def cash_flow_matrix():
    ...
    Outputs:
        - 
    TODO's:
        - This shouldn perform a summatory?
    '''
    price = aircraft_price()
    _,deliveries = delivery_forecast()
    costs = cost_matrix()
