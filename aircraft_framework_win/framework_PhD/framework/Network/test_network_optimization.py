...
arrivals_2 =  ['CD1','CD2','CD3','CD4','CD5','CD6','CD7','CD8','CD9','CD10']
...
prob += lpSum(revenue_ik) - lpSum(nika[(i,k)]*2*doc[(i,k)] for i in departures for k in arrivals if i != k)
...
for i in departures:
    for j in arrivals_2:
        for k in arrivals:
            if i != j:
                prob += lpSum(xijk[(i,j,k)]  for k in arrivals) == demand[i][j]
