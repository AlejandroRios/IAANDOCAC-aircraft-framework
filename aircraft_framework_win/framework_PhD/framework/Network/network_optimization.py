...
def network_optimization(distances,demand,DOC):
    ...
    final_airport =  ['CD1','CD2','CD3','CD4','CD5','CD6','CD7','CD8','CD9','CD10']
    ...
    for i in departure_airport:
        for k in first_stop_airport:
            if i != k:
                revenue_ik[(i,k)] = round(revenue(demand[i][k],distances[i][k],pax_capacity,pax_number))
            else:
                revenue_ik[(i,k)] = 0
    ...
    problem += lpSum(revenue_ik) - lpSum(nika[(i,k)]*2*doc[(i,k)] for i in departure_airport for k in first_stop_airport if i != k)
    ...
    for i in departure_airport:
        for j in final_airport:
            for k in first_stop_airport:
                if i != j:
                    problem += lpSum(xijk[(i,j,k)]  for k in first_stop_airport) == demand[i][j]
    ...
    for i in departure_airport:
        for j in final_airport:
            for k in first_stop_airport:
                if i != k:
                    problem += lpSum(xijk[(i,j,k)] for j in final_airport) <= G[(i,k)]
    ...
    for i in departure_airport:
        for j in final_airport:
            for k in first_stop_airport:
                if k != j:
                    problem += lpSum(xijk[(i,j,k)] for i in departure_airport) <= G[(j,k)]
