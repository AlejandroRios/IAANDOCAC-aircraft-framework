...
def TURB(Tti, f, AidAe, Mi, Me, eta_ti, TteR):
    ...
    while True: # Label 1
        ...
        _,hte, Prte, phite, cpte, Rte, gammate, ate = FAIR(item=1,f=f,T=T_te)

        # T, h, Pr, phi, Cp, R, gamma, a

        pi_t = (MFPi/MFPe)*AidAe*np.sqrt(T_te/Tti)
        Prtei = pi_t*Prti
        Ttei, htei, _, phitei, cptei, Rtei, gammatei, atei = FAIR(item=3,f=f,Pr=Prtei)
        hte = hti - eta_ti*(hti - htei)
        Tau_t = (hte/hti)
        Tten, _, Prte, phite, cpte, Rte, gammate, ate = FAIR(item=2,f=f,h=hte)
